...
选中的 review gate 不能只靠 `review/*.md` 文件存在来过关。每个选中门禁还必须有
对应 reviewer 的 `agent-results/*.json` 且 `status: PASS`；review artifact 里
保留 `PASS / FAIL`、HTML 注释、模板变量、尖括号或方括号占位时，
`validate_review_gates.py` 会 FAIL。
...
当 `Execution Mode Decision` 选中 `single Trellis subagent`、`Trellis
subagents`、`Trellis-native parallel + worktree` 或 `OMC ulw/ultrawork +
worktree + parent/child` 时，最终验证要求同时存在
`trellis-implementer` 和 `trellis-checker` 的合法 agent result。implementer
自己运行 lint/test 不等于独立 checker gate。
...
2. `validate_review_gates.py` — 检查 mandatory gates 是否选中、review 文件是否存在且有结论、是否仍有模板占位、以及每个 selected review gate 是否有匹配的 PASS reviewer agent result
3. `validate_delivery_sync.py` — 检查代码里已移除的公开路径是否还残留在 README / docs 中
4. Agent result 检查通过 `validate_task.py` 间接触发；当 `Execution Mode Decision` 选中 `single Trellis subagent`、`Trellis subagents`、`Trellis-native parallel + worktree` 或 `OMC ulw/ultrawork + worktree + parent/child` 时，必须有合法 `trellis-implementer` 与 `trellis-checker` agent result
...
.trellis/
  ...
  replay/                  ← workflow replay fixtures（25 cases）
  tasks/                   ← 活跃和已归档任务
  workspace/               ← 个人开发者日志
```
