...
- **运行时硬化** — hook schema 标准化、hard block guardrails、static validators
...
**Q: Hook 阻断了我需要的操作怎么办？**
A: Hooks 区分 hard block（危险操作）和 soft warning（编辑共享类型）。Soft warning 可带原因绕过。Hard block 不可绕过。
...
本版本对 Claude Code runtime 做了以下硬化：

- **Hook schema 标准化** — 所有 hooks 统一 `{matcher, hooks: [{type, command}]}` 结构
- **命名一致性** — agent/skill/hook/workflow 名称全局统一
- **硬阻断护栏** — stop-guard、subagent-stop-guard、protect-dangerous-actions 返回 block/deny
- **Subagent 输出契约** — 每个 agent 有明确的输出格式要求
- **Review gate 完成检测** — runtime 可检测 selected gates 是否完成
- **Validation 完成检测** — runtime 可检测 build/test 是否完成
...
团队试点前建议执行真实 Claude Code smoke test。详见 `docs/runtime-hardening.md`。
